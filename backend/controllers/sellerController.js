...
exports.forgotPassword = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!user) {
    return next(new ErrorHander("Seller not found", 404));
  }
...
// update User Profile
exports.updateSellerProfile = catchAsyncErrors(async (req, res, next) => {
  const newUserData = {
    name: req.body.name,
  ...
  const user = await Seller.findByIdAndUpdate(req.user.id, newUserData, {
    new: true,
    runValidators: true,
...
// Get all users(admin)
exports.getAllSeller = catchAsyncErrors(async (req, res, next) => {
...
exports.getSingleSeller = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new ErrorHander(`Seller does not exist with Id: ${req.params.id}`)
    );
  }
...
exports.updateSellerRole = catchAsyncErrors(async (req, res, next) => {
  ...
  await Seller.findByIdAndUpdate(req.params.id, newUserData, {
    new: true,
    runValidators: true,
    useFindAndModify: false,
  });
...
exports.deleteSeller = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new ErrorHander(`Seller does not exist with Id: ${req.params.id}`, 400)
    );
  }
